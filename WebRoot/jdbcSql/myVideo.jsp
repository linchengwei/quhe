...
function handlePaginationClick(new_page_index, pagination_container) {
    $("#stuForm").attr("action", "<%=context%>/VideoShowServlet.do?pageNum="+ (new_page_index + 1));
		$("#stuForm").submit();
		return false;
	}
...
<body>
	<div style="margin-left: 100px; margin-top: 100px;">
		...
		<div>
			<form action="<%=context%>/VideoShowServlet.do" id="stuForm"
				method="post">
		...
		<c:if test="${fn:length(result.dataList) gt 0 }">
			<table border="1px" cellspacing="0px"
				...
				<c:forEach items="${result.dataList }" var="video">
					<tr>
						...
						<td><c:out value="${video.video_address }"></c:out></td>
						<td><c:out value="${video.release_time }"></c:out></td>
						<td>
							<c:if test="${ video.check_status eq 1}">审核通过</c:if>
							<c:if test="${ video.check_status eq 0}">审核未通过</c:if>
						</td>
					</tr>
			...
			<c:forEach items="${result.dataList }" var="video">
				<div class="inbox">
					<embed src='<c:out value="${video.video_address }"></c:out>'
						width="100%" height="550px" type="application/x-shockwave-flash"
			...
			<c:forEach items="${result.dataList }" var="video">
				<div class="inbox">
					<iframe height=498 width=1000 src='${video.video_address }' frameborder=0 'allowfullscreen'></iframe>
				</div>
			</c:forEach>
